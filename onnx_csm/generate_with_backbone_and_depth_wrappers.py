#!/usr/bin/env python3
# generate_with_backbone_and_depth_wrappers.py
#
# Uses backbone + depth decoder one-step wrappers (PyTorch) and compares to HF for sanity.
...
# -------------------------
# Backbone cached step (wrapper)
# -------------------------
@torch.no_grad()
def step_backbone_with_wrapper(wrapper, new_embeds, attn_mask_2d, past_kv, seq_len):
    bsz, n_new, _ = new_embeds.shape
    ...
    for i in range(n_new):
        token_embed = new_embeds[:, i : i + 1, :]
        new_mask = torch.ones(bsz, 1, device=attn_mask_2d.device, dtype=attn_mask_2d.dtype)
        attn_mask_2d = torch.cat([attn_mask_2d, new_mask], dim=1)
        attn_mask = (1.0 - attn_mask_2d).to(dtype) * -1e4
        attn_mask = attn_mask[:, None, None, :]
        cache_pos = torch.tensor([seq_len], device=device, dtype=torch.long)
        logits, h_last, *past_kv = wrapper(token_embed, attn_mask, cache_pos, *past_kv)
        seq_len += 1
    return h_last, past_kv, seq_len, attn_mask_2d


def static_cache_to_past(cache, seq_len):
    past = []
    for layer in cache.layers:
        past.append(layer.keys[:, :, :seq_len, :].clone())
        past.append(layer.values[:, :, :seq_len, :].clone())
    return past


# -------------------------
# Depth decoding with wrapper
# -------------------------
@torch.no_grad()
...
def main():
    ...
    COMPARE_FIRST_FRAME_GREEDY = True
    COMPARE_FIRST_BACKBONE_STEP = True
    ...
    bb_wrapper = BackboneOneStepExport(model).eval().to(device)
    ...
    attn_mask_hf = inputs["attention_mask"].clone()           # [B,T0]
    attn_mask_wr = attn_mask_hf.clone()
    ...
    bb_past = static_cache_to_past(bb_pkv, seq_len)
    seq_len_hf = seq_len
    ...
    while cur_step < MAX_STEPS:
        ...
        if bool(reached_eos) or cur_step == MAX_STEPS - 1:
            ...
            h_last, bb_past, seq_len, attn_mask_wr = step_backbone_with_wrapper(
                bb_wrapper, inject_embeds, attn_mask_wr, bb_past, seq_len
            )
            continue
        ...
        h_last, bb_past, seq_len, attn_mask_wr = step_backbone_with_wrapper(
            bb_wrapper, audio_embeds, attn_mask_wr, bb_past, seq_len
        )

        if COMPARE_FIRST_BACKBONE_STEP and not backbone_checked:
            h_last_hf, bb_pkv, seq_len_hf, attn_mask_hf = step_backbone_with_cache(
                model, audio_embeds, attn_mask_hf, bb_pkv, seq_len_hf
            )
            same = torch.allclose(h_last_hf, h_last, atol=1e-2, rtol=1e-2)
            print("COMPARE backbone step:", "MATCH" if same else "MISMATCH")
            backbone_checked = True
    ...
    sf.write("output_wrappers.wav", audio.numpy(), sr)
    print("Wrote output_wrappers.wav")
